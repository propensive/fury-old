...
FURYSTABLE="QmTRpc8wSsDiWo8n1oqXLdjZ33x92R6scQmLGd262mXtNh"
...
tmp/.bundle.ipfs: dist/fury-$(VERSION).tar.gz
	@echo -n "Adding $(shell tput sitm)Fury $(VERSION)$(shell tput sgr0) TAR GZIP file to IPFS..."
	@ipfs add -q "$<" > "$@" && \
		echo "done"
...
publish: dist/fury-$(VERSION).tar.gz tmp/.bundle.ipfs .pinata/apiKey .pinata/secretApiKey
	...
	@curl -s \
		...
		"https://api.pinata.cloud/pinning/addHashToPinQueue" > /dev/null && \
		echo "done" && \
		echo "$(shell tput bold)Fury TAR GZIP file version $(VERSION) published to $(shell cat tmp/.bundle.ipfs)$(shell tput sgr0)"
