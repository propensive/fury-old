...
su -p "$USER" -c "touch $HOME/.${SHELL_NAME}rc"
su -p "$USER" -c "${SHELL_NAME}" -c /install.sh
su -p "$USER" -c "${SHELL_NAME}" -c "source /home/${SHELL_NAME}_user/.${SHELL_NAME}rc ${JOIN} fury start ${JOIN} fury about ${JOIN} fury stop"
