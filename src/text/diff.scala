/*
    ...
    Fury, version 0.18.17. Copyright 2018-20 Jon Pretty, Propensive OÜ.
...
object Diff extends Diff_1 {
  ...
  implicit def traversableDiff[T: Diff: MsgShow: EntityName]
    ...
    val rightOnly: Set[T] =
      (rightSet.map(stringShow.show) -- leftSet.map(stringShow.show)).flatMap { id =>
        rightSet.find(stringShow.show(_) == id).to[Set]
      }

    val common = (leftSet -- leftOnly).to[List].sortBy(stringShow.show) zip
      (rightSet -- rightOnly).to[List].sortBy(stringShow.show)

    (common.flatMap { case (l, r) =>
      implicitly[Diff[T]].diff(l, r).map { d =>
        d.copy(label = msg"${implicitly[MsgShow[T]].show(l)} » ${d.label}")
      }
    }) ++
      (leftOnly.map { e =>
