...
case class Layout(home: Path, pwd: Path, env: Environment) {
  ...
  private def findProjectBaseDir(here: JPath): JPath = {
    val fileSystem = Files.getFileStore(here)
    Stream.iterate(here.toAbsolutePath)(_.getParent).takeWhile { p =>
      println(p)
      Option(p).exists(Files.getFileStore(_) == fileSystem)
    }.find(_.resolve("layer.fury").toFile.isFile).ascribe(new IllegalStateException("Fuck you!")).get
  }
