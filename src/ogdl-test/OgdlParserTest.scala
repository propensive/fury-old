...
object OgdlParserTest extends TestApp {
  ...
  private[this] implicit def buffer(string: String): ByteBuffer = wrap(string.getBytes())
  private[this] implicit def graph(values: (String, Ogdl)*): Ogdl = Ogdl(values.toVector)
  private[this] implicit def leafToTuple(name: String): (String, Ogdl) = name -> empty
  private[this] implicit def leafToGraph(name: String): Ogdl = graph(name -> empty)
}
